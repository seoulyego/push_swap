...
/*   Updated: 2023/01/13 08:07:42 by yeongo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	change_rot_count(int rot[2], int rot_tmp[2])
{
	...
	unsigned int	abs_rot[2];
	unsigned int	abs_tmp[2];

	abs_rot[ST_A] = ft_abs_int(rot[ST_A]);
	abs_rot[ST_B] = ft_abs_int(rot[ST_B]);
	abs_tmp[ST_A] = ft_abs_int(rot_tmp[ST_A]);
	abs_tmp[ST_B] = ft_abs_int(rot_tmp[ST_B]);
	if ((rot[ST_B] > 0 && rot[ST_A] > 0)
		|| (rot[ST_B] < 0 && rot[ST_A] < 0))
		sum_rot = (abs_rot[ST_B] * (abs_rot[ST_B] >= abs_rot[ST_A]) \
			+ abs_rot[ST_A] * (abs_rot[ST_B] < abs_rot[ST_A]));
	else
		sum_rot = abs_rot[ST_B] + abs_rot[ST_A];
	if ((rot_tmp[ST_B] > 0 && rot_tmp[ST_A] > 0)
		|| (rot_tmp[ST_B] < 0 && rot_tmp[ST_A] < 0))
		sum_tmp = (abs_tmp[ST_B] * (abs_tmp[ST_B] >= abs_tmp[ST_A]) \
			+ abs_tmp[ST_A] * (abs_tmp[ST_B] < abs_tmp[ST_A]));
	else
		sum_tmp = abs_tmp[ST_B] + abs_tmp[ST_A];
	return (sum_rot > sum_tmp);
}
